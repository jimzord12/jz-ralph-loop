...
| 01    | [Non-Agent Commands](./01-SLICE-non-agent-commands.md)                                   | planned | 00         | `init`; `docs`; `loop create`; `validate`; `loop list`; `loop status` |
| 02    | [Run Foundations Without Codex Launch](./02-SLICE-run-foundations-no-codex.md)           | planned | 01         | `run demo`; `validate demo`                                           |
| 03    | [Agent Invocation And Artifact Capture](./03-SLICE-agent-invocation-artifact-capture.md) | planned | 02         | `bun test`; `run demo`                                                |
