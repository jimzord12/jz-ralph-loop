...
- **CONTROL_DIR** = the directory that contains this file. Its absolute path was
  ...
  - `CONTROL_DIR/PROGRESS.md` — status: a checklist of tasks grouped by phase.
...
1. Read `CONTROL_DIR/PROGRESS.md`. Find the FIRST unchecked box: a line like
...
6. Finalize the task (only if gates are green):
   - Check the box for THIS task only in `CONTROL_DIR/PROGRESS.md`: `- [ ]` →
...
- Never delete or reorder tasks in `PROGRESS.md`; only flip your own box.
...
history greppable and maps 1:1 to `PROGRESS.md`. The commit is atomic — it
includes the code, the `PROGRESS.md` box flip, and the rewritten `HANDOFF.md` —
